...
droot = root+'classifiers/v4/distributions/'

# Load P(T|W) distribution
with open(droot+'T_WW.pickle',mode='rb') as file:
    t_wwd = load(file)
...
t_wwd2 = dict()
for k,v in t_wwd.items():
    if '-' in k[1]:
        [a,b] = k[1].split('-')
        put(t_wwd2,(k[0],a,k[2]),v/2)
        put(t_wwd2,(k[0],b,k[2]),v/2)
    else:
        put(t_wwd2,k,v)
t_wwd = t_wwd2
