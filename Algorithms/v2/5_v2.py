...
droot = root+'classifiers/v2/distributions/'

# Load P(T|W) distribution
with open(droot+'T_WWT.pickle',mode='rb') as file:
    t_wwtd = load(file)
...
t_wwtd2 = dict()
for k,v in t_wwtd.items():
    if '-' in k[1]:
        [a,b] = k[1].split('-')
        put(t_wwtd2,(k[0],a,k[2],k[3]),v/2)
        put(t_wwtd2,(k[0],b,k[2],k[3]),v/2)
    else:
        put(t_wwtd2,k,v)
t_wwtd = t_wwtd2
...
for k,v in t_wwtd.items():
    k2 = (k[0],k[2],k[3])
    if k2 in prd:
        if t_wwtd[(k[0],prd[k2],k[2],k[3])] < v:
            prd[k2] = k[1]
    else:
        prd[k2] = k[1]
